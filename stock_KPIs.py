...
class KPIs:
    ...
    def yearl_risk_return(self):
        ...
        daily_retruns = data.pct_change()
        # Calculate cumulative return
        cumulative_return = (1 + daily_retruns ).prod() - 1
        total_return = cumulative_return
        ## Volatile in That time
        total_volatile = daily_retruns.std()
        assets = pd.concat([total_return, total_volatile], axis=1)
